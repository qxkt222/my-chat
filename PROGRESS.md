...
#### 7.11.8 全局条目名册（2026-09-25，问答驱动改造）

开发者需求：把预设从「角色卡绑一套（互斥）」改成酒馆那样的**全局条目名册**——
逐条开关、叠加生效、包内可排序。六个设计决定全部来自问答，逐条落成规格：

| 问答项 | 选定 | 落地 |
|---|---|---|
| 作用范围 | 全局 | 开关与顺序存在全局索引，**不读写角色卡 presetId** |
| 默认态 | 读酒馆自带标记 | 导入器读 `prompt_order` 的 `enabled` |
| 与角色关系 | **与角色无关** | 角色只管自己的 system_prompt 与世界书；名册是通用层 |
| 超预算 | 显示+提醒，不阻止 | 汇总行标红，不禁用发送 |
| 默认组合（6.5 万字） | 默认关 | `enabled=false`，保留可见 |
| 排序 | 包内可上下移 | 包内排序持久化到 `roster-index.json` |
| 内置预设 | 也进同一张表 | 与导入条目同处一张名册 |
| 那个「启用」按钮 | 改装成开关 | 从「绑角色卡」改为**全局开/关**（可开可关两态） |

**数据模型**：`PromptPreset` 加 `enabled`/`group`/`order` 三个**可选**字段；
新增 `PresetRosterIndex`（`presets/roster-index.json`）作为「包顺序 + 包内顺序」的全局锚。

⚠️ **「已启用」只认 `=== true`**：旧数据没有这个键，真值判断会把那条 6.5 万字的
「（默认组合）」默认开上，发送链路当场顶爆上下文预算。这条用单测钉死。

**旧数据迁移的诚实边界**：迁移只能补 `group`/`order`，**补不出当年的启用标记**——
那个信息在当年导入时就被丢掉了。所以迁移一律按未启用处理，宁可全关也不误开。
要拿回真实的启用状态，需要**重新导入一次原始预设文件**（新导入器会读标记）。

**真盘面实测**（非推断）：
- 启动一次真实 exe 触发迁移 → 34 个旧条目的三个键全覆盖、`ORDERS_UNIQUE=true`、
  「默认组合」`enabled=false`，并生成 `roster-index.json`（`groupOrder=["导入预设"]`，该包 34 条）
- 判据脚本 `scripts/preset-roster-check.mjs` 直接读 `%APPDATA%` 的真实 JSON：
  迁移前 RED、迁移后 GREEN。⚠️ 原判据里「至少一条 `enabled=true`」**在旧数据上永不
  可能满足**（当年没存标记），已收紧为「仅在存在新导入条目时才判」——
  这是判据自身的口径错误，不是代码不达标。
- 迁移前把 34 个原始文件备份到 `D:\1233344\presets-backup-20260925-101208`

**真浏览器验证**（`src/probe/`，jsdom 量不到布局）：故意把包内数组顺序写成 `[2,0,1]`，
实测 UI 按 `order` 渲染（条目1→2→0）、未分组垫底、整包开关与逐条开关都在、
每条显示字数、展开长模板后底部按钮仍可见（滚动容器 `min-h-0` 生效）。

**测试**：前端 85 → 121 用例（名册纯函数 22 条 + 导入器 9 条 + 发送链路名册 5 条，
其余为把既有弹窗用例改写为新语义）。

#### 7.11.9 酒馆正则套件：从「误报角色卡」到「完全导入」（2026-09-25）

**事故**：开发者把 `FF5 Regex 3.0 Suite.json` 当预设导入，应用回「这是角色卡」。

**根因（两条）**：
1. 该文件根节点是 JSON **数组**（25 条正则脚本，字段 `findRegex`/`replaceString`/
   `placement`/`disabled`/`markdownOnly`/`promptOnly`）—— 是**第四种文件类型**，
   既不是预设、不是角色卡、也不是世界书，应用原本没有这一类解析器。
2. `parseCharacterJson` 收到数组时直接取 `data.name`/`data.description`，
   全部 `undefined` → `String(undefined ?? "")` 得空串，`cardFromData` 于是
   「成功地」造出一张**空白角色卡**，被兜底分支报成「这是角色卡」。

**修法**：
- `character-card.ts` 加硬守卫：根必须是普通对象（数组/字符串/数字/null 一律拒），
  V2 的 `data` 层同样拒数组。配 4 条单测，含「真 V2 卡仍能解析」防误伤。
- 新增 `lib/regex-import.ts`：识别 + 解析正则套件
  （识别判据用「**每一项**都有 findRegex」，不是只看第一项）
- `/pattern/flags` 按**最后一个未转义的斜杠**切分（pattern 里可能含 `\/`）
- `RegexRule` 加 `flags`：实测该套件 22/25 条带 `i`，硬编码 `g` 会让规则
  「看起来导入了、实际匹配不上」—— 比报错更糟
- 「样式」tab 新增「导入正则」；预设页的识别链插入正则分支

**⚠️ 一次计数纠错（我自己说错的）**：
我最初按 `promptOnly` 字段统计，报告「20 条能显示、5 条做不到」。**这是错的。**
精确统计 `placement` 后：

| 指标 | 值 | 含义 |
|---|---|---|
| 含 placement 2 | **25** | 全部 25 条都作用于「发给模型的消息」 |
| 含 placement 1 | **2** | 只有那 2 条 `[2,1]` 也作用于显示 |
| 只有 placement 2 | 23 | 剩下 23 条**只**改出站，不改显示 |

所以这是一套**出站清理套件**，不是「显示美化」套件。按 `promptOnly` 字段分是错误的
口径 —— 那个字段与 `placement` 并不一一对应。

**完全导入的实现**（开发者要求「那 5 条也必须搞」）：
新增 `lib/outgoing-regex.ts` —— 出站通路，在 `shrinkMessages` 之后、`streamChat`
之前跑，剥掉 `<internal_states>` / GFX 块 / `<!-- IMG_PROMPT:… -->` / 思考块。
关键约束：
- **只作用于待发副本**，绝不回写 `conv.messages`，界面显示不受影响（与酒馆一致）
- 尊重酒馆的 `minDepth`/`maxDepth`：实测 Context Saver 带 `minDepth=2`，
  只清「距末尾 ≥2 条」的历史，最新两条保留 —— 这不是我们发明的语义
- 四条酒馆发送分支（单聊/群聊/续写…）全部接入

**验证**（真文件，非合成数据）：
- `src/lib/real-suite-check.test.tsx` 直接读开发者磁盘上那个文件（缺失时整组 skip），
  断言：25 条全导入、placement 分布 25/2、不再被误判成角色卡、出站剥净而显示不受影响
- 合成数据（3 条）**没能**暴露这个问题 —— 所以留了这条真实文件回归

**测试**：前端 142 → 169 用例（新增 `outgoing-regex.test.ts` 19 条、
`real-suite-check.test.tsx` 5 条，并把 `regex-import.test.ts` 的旧断言
「promptOnly 应被跳过」反过来改为「应被导入」）。

#### 7.11.10 三个面板的常驻返回（吸顶头，2026-09-25）

开发者反馈（原话）：「就单单样式这个功能我用后就发现**由于导入了很多东西导致上面那些
排序被一起挤走了**，就想在**不改动**（弹窗大小）情况下加入返回功能，就像预设那样处理」。

**问题本质不是「缺按钮」，是「按钮会被滚走」**：`SettingsDialog` 的内容区是**唯一**
的滚动容器（`flex-1 overflow-y-auto`），面板标题行与「返回/新建/导入」都在它里面 ——
条目一多（实测样式页 60 条规则、滚动高度 2347px）就随内容滚出视口。

**解法**：新建 `components/settings/PanelHeader.tsx`，把标题行做成
`sticky top-0 z-10` + 不透明底色 + 负外边距抵消父级 `p-4`；三个面板
（预设/世界书/样式）共用同一实现。**弹窗尺寸与滚动结构一概未动**。

三个容易踩的点都写进组件注释：必须给**不透明**底色（否则滚动时内容从标题下透出）、
负外边距抵消 `p-4`（否则吸顶后两侧留白不一致）、`z-10` 压住内容。

**真浏览器实测**（jsdom 元素高度恒为 0、`position:sticky` 不生效，量不到这类问题）：

```
滚动容器 top=112  clientH=344  scrollH=2347
  → 滚到底 scrollTop=2003
「返回」按钮: top=144  可见=true
```

滚了 **2003px** 之后返回按钮仍在 `top=144`（紧贴滚动容器顶部）。**吸顶若失效，
它的 top 会是 144−2003 ≈ −1859，完全不可见** —— 这是与「没做」的可判分差别。
探针留在 `src/probe/sticky-probe.{html,tsx}`（造 60 条规则 + 40 条世界书条目）。

**测试**：`SettingsDialog.test.tsx` 新增 3 条（预设/世界书/样式各一条），
断言「返回存在 + 点了切回上一层 tab + 弹窗不关」。⚠️ 其中世界书那条一开始写错了：
我用「导入」当按钮标志，但那个词在角色页也有，切回去后不会消失 ——
改成**直接读当前选中 tab**（带 `border-primary` 的那个）才是不依赖文案的判据。
前端 169 → 172 用例。
